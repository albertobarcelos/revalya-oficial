/**
 * AIDEV-NOTE: Componente para exibição do total calculado do serviço
 * Mostra subtotal, desconto e total final
 * 
 * @module features/contracts/components/ContractServices/ServiceTotalDisplay
 */
...
interface ServiceTotalDisplayProps {
  /** Valor unitário */
  unitPrice: number;
  /** Quantidade */
  quantity: number;
...
/**
 * Exibe o total calculado do serviço com desconto
 * 
 * @example
 * ```tsx
 * <ServiceTotalDisplay
 *   unitPrice={draftUnitPrice ?? currentService?.unit_price ?? 0}
 *   quantity={draftQuantity ?? currentService?.quantity ?? 1}
 *   discountData={discountData}
 * />
 * ```
 */
export function ServiceTotalDisplay({ 
  unitPrice, 
  quantity, 
  discountData 
}: ServiceTotalDisplayProps) {
  // Calcular valores
...
export default ServiceTotalDisplay;
