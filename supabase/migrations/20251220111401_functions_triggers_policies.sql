-- ============================================
-- MIGRAÇÃO: Estruturas do Banco (sem tabelas)
-- Extraído automaticamente dos arquivos de migração
-- ============================================
...
DO $$ 
BEGIN
    -- Tentar remover todas as variações possíveis da função
    DROP FUNCTION IF EXISTS "public"."get_bank_statement"("uuid", "uuid", "date", "date", "public"."bank_operation_type") CASCADE;
...
COMMENT ON FUNCTION "public"."user_has_contract_service_access"("p_contract_id" "uuid", "p_tenant_id" "uuid") IS 'FUNÇÃO DE SEGURANÇA MULTI-TENANT - CONTRACT_SERVICES. Valida se o usuário atual tem acesso a um registro específico de contract_services implementando validação multi-camada para garantir isolamento de dados.';
...
-- ============================================
-- POLICIES (RLS) - REMOVIDAS (não necessárias)
-- ============================================

-- Todas as políticas RLS foram removidas conforme solicitado

-- ============================================
-- GRANTS (Functions, Sequences, Types, Schemas)
-- ============================================
...
-- Criar role apenas se não existir
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'cli_login_postgres') THEN
        CREATE ROLE "cli_login_postgres";
    END IF;
END $$;

ALTER ROLE "cli_login_postgres" WITH NOINHERIT NOCREATEROLE NOCREATEDB LOGIN NOBYPASSRLS VALID UNTIL '2025-12-20 05:12:31.942899+00';
...
-- Removido: requer privilégios de superuser
-- Se necessário, execute manualmente como superuser
