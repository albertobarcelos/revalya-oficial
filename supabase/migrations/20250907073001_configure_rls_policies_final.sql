...
-- Verifica a estrutura da tabela antes de criar índices
DO $$ 
DECLARE
  column_exists BOOLEAN;
BEGIN
  -- Verificar se a coluna user_id existe na tabela audit_logs
  SELECT EXISTS (
  ...
  IF column_exists AND EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'audit_logs'
    AND column_name = 'tenant_id'
  ) THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_audit_logs_user_tenant ON public.audit_logs (user_id, tenant_id)';
  END IF;
  ...
  -- Verificar se a coluna action existe na tabela security_logs
  SELECT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' 
  ...
  IF column_exists THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_security_logs_action ON public.security_logs (action)';
  END IF;
END $$;
