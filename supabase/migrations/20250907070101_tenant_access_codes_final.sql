-- Script revisado para evitar erros de conflito
-- Verificar se a tabela já existe
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tenant_access_codes') THEN
    ...
    CREATE INDEX idx_tenant_access_codes_code ON public.tenant_access_codes (code);

    -- Índice para busca por tenant_id e user_id
    CREATE INDEX idx_tenant_access_codes_tenant_user ON public.tenant_access_codes (tenant_id, user_id);

    -- Índices para performance de validação e limpeza
    CREATE INDEX idx_tenant_access_codes_expires_at ON public.tenant_access_codes (expires_at);
    CREATE INDEX idx_tenant_access_codes_used_at ON public.tenant_access_codes (used_at);
...
-- Adicionar coluna de token_version na tabela tenant_users para revogação (seguro para repetir)
