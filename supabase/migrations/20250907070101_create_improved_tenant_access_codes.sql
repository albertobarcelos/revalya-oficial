...
-- Criar job para limpar códigos expirados automaticamente
-- Note: isso requer pg_cron extension que pode não estar disponível em todos os planos do Supabase
-- Se pg_cron não estiver disponível, remova este bloco
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.schedule(
      'cleanup-expired-tenant-codes',
      '0 * * * *',  -- A cada hora
