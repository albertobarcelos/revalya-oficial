...
async function checkTenantIntegration(tenantId: string, environment: string = 'production'): Promise<boolean> {
  try {
    ...
    const { data: allIntegrations, error: allError } = await supabase
      ...
      .select('id, is_active, environment, integration_type, created_at, updated_at')
    ...
    console.log(`[checkTenantIntegration] Primeira tentativa (sem filtro environment, is_active=true):`, {
      found: !!data,
      error: error?.message,
      data: data ? { is_active: data.is_active, environment: data.environment } : null
    });
    ...
    if (error || !data) {
      ...
      console.log(`[checkTenantIntegration] Segunda tentativa (com filtro environment=${environment}, is_active=true):`, {
        ...
        data: result.data ? { is_active: result.data.is_active, environment: result.data.environment } : null
      });
      
      if (result.error || !result.data) {
        // AIDEV-NOTE: Tentar buscar sem filtro de is_active para ver se existe mas está inativa
        const inactiveCheck = await supabase
          .from('tenant_integrations')
          .select('is_active, environment')
          .eq('tenant_id', tenantId)
          .eq('integration_type', 'whatsapp')
...
serve(async (req) => {
  ...
  try {
    ...
    if (contentType?.includes('application/json')) {
      ...
      responseData = text ? { data: text } : { success: true };
    }
