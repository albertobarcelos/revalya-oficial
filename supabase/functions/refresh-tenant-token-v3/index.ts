...
const CONFIG = {
  ...
  REFRESH_TOKEN_ROTATION: false, // Rotacionar refresh token a cada renovação
...
interface RefreshTokenResponse {
  accessToken: string;
  refreshToken?: string; // Novo refresh token se rotação estiver habilitada
  expiresAt: number;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { refreshToken, tenantSlug }: RefreshTokenRequest = await req.json()
    ...
    if (refreshPayload.type !== 'refresh' || refreshPayload.tenant_slug !== tenantSlug) {
      ...
      return new Response(
        JSON.stringify({ error: 'Refresh token inválido para este tenant' }),
        { 
          status: 401, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    const { data: sessionData, error: sessionError } = await supabaseClient
      ...
      .select('id, is_active, refresh_expires_at, last_access')
      .eq('user_id', userId)
    ...
    if (accessError || !tenantAccess) {
      ...
      return new Response(
        JSON.stringify({ error: 'Acesso ao tenant foi revogado' }),
        { 
          status: 403, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (tenantError || !tenant) {
      ...
      return new Response(
        JSON.stringify({ error: 'Tenant não está mais ativo' }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
