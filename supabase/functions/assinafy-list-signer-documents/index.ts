...
interface ListSignerDocumentsRequest {
  signer_id?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const body: Partial<ListSignerDocumentsRequest> = await req.json().catch(() => ({}));
    ...
    if (!assinafyResp.ok) {
      return new Response(
        JSON.stringify({ error: "Falha ao listar documentos do signer", status: assinafyResp.status, data }),
