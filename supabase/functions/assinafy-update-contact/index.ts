...
/**
 * Handler da Edge Function assinafy-update-contact
 * Comentário de nível de função: atualiza dados de um contato (signer) na Assinafy com credenciais seguras
 */
serve(async (req) => {
  ...
  try {
    ...
    const body: Partial<UpdateContactRequest> = await req.json().catch(() => ({}));
    const signerId = typeof body.signer_id === "string" ? body.signer_id : undefined;

    if (!signerId) {
      return new Response(JSON.stringify({ error: "signer_id obrigatório" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!assinafyResp.ok) {
      return new Response(
        JSON.stringify({ error: "Falha ao atualizar contato", status: assinafyResp.status, data }),
