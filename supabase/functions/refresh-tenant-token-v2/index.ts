...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!refreshToken || !tenantSlug) {
      return new Response(
        JSON.stringify({ error: 'Parâmetros obrigatórios: refreshToken, tenantSlug' }),
        { 
          status: 400, 
    ...
    if (!tokenData.tenants.active) {
      return new Response(
        JSON.stringify({ error: 'Tenant inativo' }),
        { 
          status: 403, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (accessError || !tenantAccess) {
      return new Response(
        JSON.stringify({ error: 'Usuário não tem mais acesso a este tenant' }),
        { 
          status: 403, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
