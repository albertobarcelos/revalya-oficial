...
async function checkInvitePermission(supabase, inviterUserId, resellerId) {
  ...
  const { data, error } = await supabase.from('reseller_users').select('user_id').eq('reseller_id', resellerId).eq('user_id', inviterUserId).maybeSingle();
  if (error) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', // IMPORTANTE: Use service_role key aqui para ter permissão de inserir/consultar
    ...
    if (!reseller_id || !email) {
      return new Response(JSON.stringify({
        error: 'Missing reseller_id or email in request body'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (!hasPermission) {
      ...
      return new Response(JSON.stringify({
        error: 'Forbidden: You do not have permission to invite users to this reseller.'
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    const { data: userIdResult, error: emailCheckError } = await supabaseClient.rpc('get_user_id_by_email', {
    ...
    if (existingUserId) {
      const { data: existingMembership, error: membershipError } = await supabaseClient.from('reseller_users').select('id').eq('reseller_id', reseller_id).eq('user_id', existingUserId).maybeSingle();
      if (membershipError) {
      ...
      if (existingMembership) {
        return new Response(JSON.stringify({
          error: 'Conflict: This user is already a member of this reseller.'
        }), {
          status: 409,
          headers: {
            ...corsHeaders,
    ...
    const { data: existingInvite, error: inviteCheckError } = await supabaseClient.from('invites').select('invite_id').eq('email', normalizedEmail).eq('reseller_id', reseller_id) // Especificamente para este revendedor
    .eq('status', 'pending').gt('expires_at', now).maybeSingle();
    if (inviteCheckError) {
    ...
    if (existingInvite) {
      return new Response(JSON.stringify({
        error: 'Conflict: An active pending invite already exists for this email and reseller.'
      }), {
        status: 409,
        headers: {
          ...corsHeaders,
    ...
    const { error: insertError } = await supabaseClient.from('invites').insert({
      email: normalizedEmail,
      token: token,
      reseller_id: reseller_id,
      status: 'pending',
      invite_type: 'reseller_user',
    ...
    return new Response(JSON.stringify({
      message: 'Reseller user invite created successfully.'
    }), {
      status: 201,
      headers: {
        ...corsHeaders,
