...
serve(async (req) => {
  ...
  try {
    ...
    const supabaseClient = createClient(
      ...
      // IMPORTANTE: Precisa da service_role key para inserir em reseller_users e atualizar invites
    ...
    if (authError || !user) {
      ...
      return new Response(JSON.stringify({ error: 'Unauthorized: You must be logged in to accept an invite.' }), {
    ...
    const { data: invite, error: queryError } = await supabaseClient
      .from('invites')
      .select('invite_id, email, reseller_id, status, invite_type, expires_at')
      .eq('token', token)
      .single();
    ...
    if (invite.status !== 'pending') {
       return new Response(JSON.stringify({ error: `Cannot accept invite: Status is "${invite.status}".` }), {
         status: 409, // Conflict
         headers: { ...corsHeaders, 'Content-Type': 'application/json' },
       });
    }
    if (invite.invite_type !== 'reseller_user') {
       return new Response(JSON.stringify({ error: `Cannot accept invite: Invalid type "${invite.invite_type}".` }), {
         status: 400,
         headers: { ...corsHeaders, 'Content-Type': 'application/json' },
       });
    }
     if (new Date(invite.expires_at) < new Date(now)) {
      ...
      return new Response(JSON.stringify({ error: 'Cannot accept invite: Invite has expired.' }), {
        status: 410, // Gone
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const { data: existingMembership, error: membershipError } = await supabaseClient
      .from('reseller_users')
      .select('id')
      .eq('reseller_id', invite.reseller_id) // ID do revendedor do convite
    ...
    if (existingMembership) {
        ...
        // Vamos marcar como aceito para consistência e retornar sucesso.
         const { error: updateError } = await supabaseClient
          .from('invites')
          .update({ status: 'accepted', accepted_at: now }) // Opcional: guardar quando foi aceito
        ...
        return new Response(JSON.stringify({ message: 'Invite accepted successfully (user was already a member).' }), {
    ...
    const { error: insertMemberError } = await supabaseClient
      ...
      .insert({
        ...
        // created_at, updated_at terão valores padrão
      });
    ...
    if (insertMemberError) {
        ...
        if (insertMemberError.code === '23505') { // Código PostgreSQL para unique violation
    ...
    // 8. Retornar sucesso
    return new Response(JSON.stringify({ message: 'Invite accepted successfully.' }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
