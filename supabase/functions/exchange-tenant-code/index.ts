...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!slug) {
      return new Response(JSON.stringify({ error: 'Slug do tenant é obrigatório' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    if (!code) {
      return new Response(JSON.stringify({ error: 'Código de acesso é obrigatório' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    const { data: codeData, error: codeError } = await supabaseClient
      ...
      .select('id, user_id, tenant_id, code, expires_at, used_at')
      .eq('code', code)
      .single()
    ...
    if (codeError || !codeData) {
      await logSecurityEvent({
        ...
        ip_address: clientIP,
        user_agent: req.headers.get('user-agent') || '',
        details: { code_prefix: code.substring(0, 8) }
      })
