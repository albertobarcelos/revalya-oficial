...
# Gerar diff de alterações feitas no Studio
...
```bash
# Staging
supabase link --project-ref wlabxeqmryswiweofwbv
supabase db push
...
### Staging
- **API:** `https://wlabxeqmryswiweofwbv.supabase.co`
- **Dashboard:** [Link Dashboard Staging](https://supabase.com/dashboard/project/wlabxeqmryswiweofwbv)

## ⚠️ Notas Importantes

1. **Branches NÃO compartilham dados:** A branch staging começa vazia. Use `seed.sql` para dados de teste.

2. **Migrações são sequenciais:** Cada migração é aplicada na ordem dos timestamps.

3. **Rollback de migrações:** Para reverter, crie uma nova migração com as alterações inversas.

4. **Edge Functions:** São deployadas automaticamente junto com as migrações.

5. **Custo:** A branch staging tem um custo de ~$10/mês ($0.01344/hora).
