    ...
    for(let attempt = 1; attempt <= MAX_RETRIES; attempt++){
      try {
        ...
        if (data.error || data.message && !isSuccessStatus) {
          ...
          if (errorLower.includes('qrcode') || errorLower.includes('qr code') || errorLower.includes('disconnected') || errorLower.includes('desconectado') || errorLower.includes('not connected') || errorLower.includes('connection') || errorLower.includes('instance') && errorLower.includes('not found') || errorLower.includes('cannot read properties') && errorLower.includes('id')) {
        ...
        const errorMessage = typeof lastError === 'object' ? JSON.stringify(lastError) : String(lastError);
...
class BulkService {
    ...
    try {
      ...
      const insertData = {
        ...
        metadata: {
          ...
          customer_phone: payload.customerPhone || payload.phone || null,
  ...
  renderMessage(tpl, customer, charge) {
    ...
    let message = tpl// Tags de cliente
    .replace(/\{cliente\.nome\}/g, nome).replace(/\{cliente\.empresa\}/g, empresa).replace(/\{cliente\.cpf\}/g, cpfCnpj) // AIDEV-NOTE: Compatibilidade com tag {cliente.cpf}
    .replace(/\{cliente\.cpf_cnpj\}/g, cpfCnpj).replace(/\{cliente\.telefone\}/g, telefone).replace(/\{cliente\.email\}/g, email)// Tags de cobrança
    .replace(/\{cobranca\.valor\}/g, valor).replace(/\{cobranca\.vencimento\}/g, vencimento).replace(/\{cobranca\.descricao\}/g, descricao).replace(/\{cobranca\.status\}/g, status).replace(/\{cobranca\.codigoBarras\}/g, codigoBarras)// AIDEV-NOTE: Tags de pagamento
    .replace(/\{cobranca\.pix_copia_cola\}/g, pixCopiaCola).replace(/\{cobranca\.pix\}/g, pixCopiaCola) // Compatibilidade com tag antiga
    .replace(/\{cobranca\.link\}/g, cobrancaLink).replace(/\{cobranca\.link_pix\}/g, cobrancaLink) // Compatibilidade com tag antiga
    .replace(/\{cobranca\.link_boleto\}/g, linkBoleto)// AIDEV-NOTE: Tags de dias
    .replace(/\{dias\.ateVencimento\}/g, diasAteVencimento.toString()).replace(/\{dias\.aposVencimento\}/g, diasAposVencimento.toString())// Manter compatibilidade com tags antigas ({{nome}}, {{valor}}, etc.)
    .replace(/\{\{nome\}\}/g, nome).replace(/\{\{valor\}\}/g, valor).replace(/\{\{vencimento\}\}/g, vencimento).replace(/\{\{telefone\}\}/g, telefone).replace(/\{\{email\}\}/g, email);
    return message;
  }
  async run(input) {
      ...
      const workers = new Array(CONC).fill(0).map(async ()=>{
        while(queue.length){
          ...
          try {
            ...
            // AIDEV-NOTE: Prioridade: celular_whatsapp primeiro, depois phone como fallback
            const customerPhone = customer.celular_whatsapp?.trim() || customer.phone?.trim() || '';
