...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: tokenData, error: tokenError } = await supabaseClient.rpc('validate_refresh_token', {
    ...
    const { data: tenantData, error: tenantError } = await supabaseClient.from('tenants').select('id, slug, name, active').eq('id', tokenInfo.tenant_id).single();
