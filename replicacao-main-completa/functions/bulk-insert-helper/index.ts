...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { table, data, batchSize = 100, upsert = false, onConflict } = await req.json();
    if (!table || !data || !Array.isArray(data) || data.length === 0) {
      return new Response(JSON.stringify({
        error: 'Invalid request: table and data array required'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const createDetailedError = (error, batchIndex, batchData)=>{
      ...
      try {
        ...
        for(let j = 0; j < batch.length; j++){
          ...
          try {
            ...
            const detailedError = createDetailedError(recordError, recordIndex, [
              record
            ]);
