...
async function findOrCreateCustomer(supabaseUser, tenantId, asaasCustomerId, customerData, apiKey, apiUrl) {
  ...
  if (asaasCustomerId) {
    const { data: existingCustomer } = await supabaseUser.from("customers").select("id").eq("tenant_id", tenantId).eq("customer_asaas_id", asaasCustomerId).maybeSingle();
    if (existingCustomer) {
  ...
  if (customerData?.cpfCnpj) {
    const { data: existingCustomer } = await supabaseUser.from("customers").select("id").eq("tenant_id", tenantId).eq("cpf_cnpj", customerData.cpfCnpj).maybeSingle();
    if (existingCustomer) {
      // AIDEV-NOTE: Atualizar customer_asaas_id se não tiver
      if (asaasCustomerId) {
        await supabaseUser.from("customers").update({
  ...
  const { data: newCustomer, error: createError } = await supabaseUser.from("customers").insert({
    tenant_id: tenantId,
    customer_asaas_id: asaasCustomerId,
    name: customerName,
...
async function findContractByExternalReference(supabaseUser, tenantId, externalReference) {
  if (!externalReference) {
    return null;
  }
  // AIDEV-NOTE: Tentar buscar contrato pelo número ou ID na externalReference
  const { data: contract } = await supabaseUser.from("contracts").select("id").eq("tenant_id", tenantId).or(`contract_number.eq.${externalReference},id.eq.${externalReference}`).maybeSingle();
  if (contract) {
...
async function findContractByCustomerId(supabaseUser, tenantId, customerId) {
  if (!customerId) {
    return null;
  }
  // AIDEV-NOTE: Buscar contratos do customer, priorizando ATIVOS e mais recentes
  const { data: contract } = await supabaseUser.from("contracts").select("id, status, created_at").eq("tenant_id", tenantId).eq("customer_id", customerId).in("status", [
    "ACTIVE",
    "DRAFT"
...
async function importChargesFromAsaas(request, supabaseUser, userId) {
  ...
  const { data: integration, error: integrationError } = await supabaseAdmin.from('tenant_integrations').select('*').eq('tenant_id', tenant_id).eq('integration_type', 'asaas').eq('is_active', true).single();
  ...
  let totalErrors = 0;
  let hasMore = true;
  const errors = []; // AIDEV-NOTE: Array para coletar erros detalhados
  while(hasMore && totalProcessed < TOTAL_LIMIT){
    ...
    for (const payment of payments){
      ...
      try {
        ...
        const { data: existingCharge } = await supabaseAdmin.from('charges').select('id, status, valor, data_pagamento, updated_at').eq('tenant_id', tenant_id).eq('asaas_id', payment.id).maybeSingle();
        ...
        const hasChanges = existingCharge && (existingCharge.status !== mappedStatus || existingCharge.valor !== payment.value || existingCharge.data_pagamento !== payment.paymentDate);
        ...
        const customerUuid = await findOrCreateCustomer(supabaseUser, tenant_id, payment.customer, customerData, api_key, api_url);
        if (!customerUuid) {
        ...
        let contractId = await findContractByExternalReference(supabaseUser, tenant_id, payment.externalReference);
        // AIDEV-NOTE: Se não encontrou por externalReference, buscar por customer_id
        if (!contractId && customerUuid) {
          contractId = await findContractByCustomerId(supabaseUser, tenant_id, customerUuid);
        }
        ...
        const { data: charge, error: chargeError } = await supabaseAdmin.from('charges').upsert(chargeData, {
  ...
  return {
    ...
    summary: {
      ...
      imported_ids: new Array(totalImported).fill(null),
      updated_ids: new Array(totalUpdated).fill(null),
      skipped_ids: new Array(totalSkipped).fill(null),
