...
async function importAllChargesFromAsaas(request) {
  ...
  const { data: integration, error: integrationError } = await supabase.from('tenant_integrations').select('*').eq('tenant_id', tenant_id).eq('integration_type', 'asaas').eq('is_active', true).single();
  ...
  while(hasMore){
    ...
    for (const payment of payments){
      try {
        ...
        const { data: existingCharge } = await supabase.from('charges').select('id, status, valor, data_pagamento, updated_at').eq('tenant_id', tenant_id).eq('asaas_id', payment.id).maybeSingle();
        ...
        const customerId = await findOrCreateCustomer(tenant_id, payment.customer, customerData, api_key, api_url);
        if (!customerId) {
        ...
        if (payment.externalReference) {
          contractId = await findContractByExternalReference(tenant_id, payment.externalReference);
        }
        if (!contractId && customerId) {
          contractId = await findContractByCustomerId(tenant_id, customerId);
        }
        ...
        if (existingCharge) {
          const { error: updateError } = await supabase.from('charges').update(chargeData).eq('id', existingCharge.id);
          if (updateError) {
  ...
  return {
    ...
    message: hasMoreCharges ? `Importação parcial: ${totalImported} novas charges importadas, ${totalUpdated} atualizadas. Execute novamente para continuar.` : `Importação completa finalizada. ${totalImported} novas charges importadas, ${totalUpdated} atualizadas.`
  };
}
