...
async function getAsaasCredentials(tenantId, environment = 'production') {
  ...
  let { data, error } = await supabase.from('tenant_integrations').select('config, is_active, environment').eq('tenant_id', tenantId).eq('integration_type', 'asaas').eq('is_active', true).maybeSingle();
  ...
  if (error || !data) {
    ...
    const result = await supabase.from('tenant_integrations').select('config, is_active, environment').eq('tenant_id', tenantId).eq('integration_type', 'asaas').eq('environment', environment).eq('is_active', true).maybeSingle();
