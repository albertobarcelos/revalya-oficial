...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { tenantId, tenantSlug, userToken } = await req.json();
    // Validar parâmetros
    if (!tenantId || !tenantSlug || !userToken) {
      return new Response(JSON.stringify({
        error: 'Parâmetros obrigatórios: tenantId, tenantSlug, userToken'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: tenant, error: tenantError } = await supabaseClient.from('tenants').select('id, slug, name, active').eq('id', tenantId).eq('slug', tenantSlug).eq('active', true).single();
    ...
    const { error: saveError } = await supabaseClient.from('tenant_refresh_tokens').insert({
      user_id: userId,
      tenant_id: tenantId,
      token: refreshToken,
