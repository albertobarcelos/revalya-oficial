...
async function getAsaasCredentials(tenantId) {
  const { data, error } = await supabase.from('tenant_integrations').select('api_key, api_url, environment').eq('tenant_id', tenantId).eq('integration_type', 'asaas').eq('is_active', true).maybeSingle();
  if (error || !data) {
...
async function fetchAsaasCustomer(customerId, credentials) {
  try {
    ...
    console.log('Dados do cliente obtidos do ASAAS:', {
      id: customerData.id,
...
serve(async (req)=>{
  ...
  try {
    ...
    console.log('Buscando dados do cliente ASAAS:', {
      customer_id,
      tenant_id
    });
